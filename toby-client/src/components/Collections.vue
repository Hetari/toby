    ...
    <div class="flex flex-col w-full">
      ...
      <div class="flex justify-between w-full pt-4 px-2 pb-4 pr-4 border-b border-rgbgray">
        ...
        <button class="hover:text-pink-500">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="chakra-icon css-sqo9k5" focusable="false">
      ...
      <div class="p-[19.2px] place-content-end flex border-b border-rgbgray">
        <button class="border rounded-md py-1 px-6 hover:opacity-80 bg-pink-600 border-pink-600 font-semibold cursor-pointer" @click="togglePopup('add')">
            ...
            >
              <div class="flex justify-between items-center">
                <div class="flex items-center">
                  ...
                  <svg v-if="collection.starred" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 ml-2 text-yellow-500" fill="currentColor" viewBox="0 0 24 24" stroke="none">
                    <path d="M12 .587l3.668 7.513 8.332 1.151-6.001 5.775 1.417 8.148L12 18.896l-7.416 4.278 1.417-8.148L0 9.251l8.332-1.151L12 .587z"></path>
                  </svg>
                </div>
                ...
                <div class="relative">
                  ...
                  <!-- Dropdown menu -->
                  <div v-if="dropdownIndex === index" class="absolute right-0 mt-2 w-48 bg-[#414150]  border border-gray-200 rounded-md shadow-lg z-20">
                    <ul>
                      <li @click="toggleStar(index)" class="px-4 py-2 hover:bg-[#4c4c5c] cursor-pointer">
                        {{ collection.starred ? 'Unstar' : 'Star' }}
                      </li>
                      <li @click="togglePopup('edit', index)" class="px-4 py-2 hover:bg-[#4c4c5c] cursor-pointer">
                        Edit
                      </li>
                      <li @click="togglePopup('tag', index)" class="px-4 py-2 hover:bg-[#4c4c5c] cursor-pointer">
                        Add Tag
                      </li>
                      <li @click="deleteCollection(index)" class="px-4 py-2 hover:bg-[#4c4c5c] cursor-pointer text-red-500">
                        Delete
                      </li>
                    </ul>
                  </div>
              ...
              <!-- Tags display -->
              <div class="mt-2">
                <span v-if="collection.tags.length > 0" v-for="(tag, idx) in collection.tags" :key="idx" class="inline-block text-sm px-2 py-1 rounded mr-2">
                  {{ tag }}
                </span>
              </div>
            </div>
    ...
    <div v-if="showPopup" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div class="bg-white rounded-lg shadow-xl p-10 w-96 text-center relative">
        <h2 class="text-2xl font-semibold mb-4 text-black">
          {{ popupMode === 'add' ? 'Add New Collection' : (popupMode === 'edit' ? 'Edit Collection' : 'Add Tag') }}
        </h2>
        ...
        <input 
          ...
          :placeholder="popupMode === 'add' ? 'Enter collection name' : (popupMode === 'edit' ? 'Edit collection name' : 'Enter tag')" 
        />
        ...
        >
          {{ popupMode === 'add' ? 'Add Collection' : (popupMode === 'edit' ? 'Save Changes' : 'Add Tag') }}
        </button>
      </div>
    </div>
  </div>
...
const togglePopup = (mode: string = '', index: number | null = null) => {
  ...
  popupInput.value = mode === 'edit' && index !== null ? collections.value[index].name : '';
};

const submitPopup = () => {
  if (popupMode.value === 'add' && popupInput.value.trim()) {
    collections.value.push({ name: popupInput.value.trim(), starred: false, tags: [] });
  } else if (popupMode.value === 'edit' && currentEditIndex.value !== null && popupInput.value.trim()) {
    collections.value[currentEditIndex.value].name = popupInput.value.trim();
  } else if (popupMode.value === 'tag' && currentEditIndex.value !== null && popupInput.value.trim()) {
    collections.value[currentEditIndex.value].tags.push(popupInput.value.trim());
  }
  togglePopup();
};
